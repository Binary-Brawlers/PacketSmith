...
impl TextInput {
    ...
    fn index_for_mouse_position(&self, position: Point<Pixels>) -> usize {
        ...
        let mut index = line
            .closest_index_for_x(position.x - bounds.left() + self.horizontal_offset)
...
impl EntityInputHandler for TextInput {
    ...
    fn character_index_for_point(
        ...
        let mut utf8_index = last_layout
            .closest_index_for_x(line_point.x + self.horizontal_offset)
            .min(self.content.len());
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let visible_width = (bounds.size.width - px(4.)).max(px(0.));
        let mut horizontal_offset = input.horizontal_offset;
        if cursor_x < horizontal_offset {
            horizontal_offset = cursor_x;
        }
        if cursor_x > horizontal_offset + visible_width {
            horizontal_offset = cursor_x - visible_width;
        }
        let cursor_pos = cursor_x - horizontal_offset;
